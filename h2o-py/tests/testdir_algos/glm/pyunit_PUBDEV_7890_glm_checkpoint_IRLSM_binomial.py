...
def buildModelCheckpointing(training_frame, x_indices, y_index, family, solver):
    ...
    model = H2OGeneralizedLinearEstimator(family=family, max_iterations=7, solver=solver, intercept=False)
    model.train(training_frame=split_frames[0], x=x_indices, y=y_index, validation_frame=split_frames[1])
    modelCheckpoint = H2OGeneralizedLinearEstimator(family=family, checkpoint=model.model_id, solver=solver, intercept=False)
